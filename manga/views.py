...
def chapter_read(request, manga_slug, volume, chapter_number):
    """
    Отображает страницу чтения главы.
    ...
    # 2. Собираем список всех глав (для боковой навигации, если нужно)
    all_chapters = Chapter.objects.filter(manga=manga) \
                                  .order_by('volume', 'chapter_number')

    # 3. Вычисляем предыдущую/следующую главу (с учётом перехода между томами)
    prev_qs = Chapter.objects.filter(
        manga=manga,
        volume=volume,
        chapter_number__lt=chapter.chapter_number
    ).order_by('-chapter_number')
    previous_chapter = prev_qs.first()
    ...
    next_qs = Chapter.objects.filter(
        manga=manga,
        volume=volume,
        chapter_number__gt=chapter.chapter_number
    ).order_by('chapter_number')
    next_chapter = next_qs.first()
    if not next_chapter:
        next_chapter = (
            Chapter.objects
            .filter(manga=manga, volume=volume+1)
    ...
    if request.user.is_authenticated:
        ...
        if not created:
            ...
            # если текущая глава «больше» последней
    ...
    # 5. Собираем WebP-страницы, если они есть
    page_images = []
    ...
    # 6. Списки по ролям (переводчики, клинеры, тайперы)
